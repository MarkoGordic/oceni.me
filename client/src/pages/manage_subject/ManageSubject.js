...
import ProfessorSearchItem from '../../components/ProfessorSearchItem/ProfessorSearchItem';
import SingleSubjectEmployee from '../../components/SingleSubjectEmployee/SingleSubjectEmployee';
import AddSubjectEmployeeModal from '../../components/AddSubjectEmployeeModal/AddSubjectEmployeeModal';
...
function ManageSubject() {
    ...
    const [subjectDataDisplay, setSubjectDataDisplay] = useState({
        name: '',
        code: '',
        professor_id: null,
        course_code: '',
    ...
    const [subjectEmployees, setSubjectEmployees] = useState([]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSubjectDataDisplay(subjectData);
    ...
    const fetchSubjectEmployees = async () => {
        try {
            const response = await fetch(`http://localhost:8000/subjects/get_employees/${id}`, {
                method: 'GET',
                credentials: 'include',
            });
            ...
            if (response.ok) {
                const data = await response.json();
                setSubjectEmployees(data);
                console.log(data);
            } else {
                toast.error('Došlo je do greške prilikom učitavanja zaposlenih.');
            }
        } catch (error) {
            console.error('Error fetching subject data:', error);
            toast.error('Došlo je do greške prilikom učitavanja zaposlenih.');
    ...
    useEffect(() => {
        if (subjectData.professor_id != null)
            fetchSubjectEmployees();
    }, [subjectData]);

    const handleRemoveSuccess = (employeeId) => {
        setSubjectEmployees(prevEmployees => prevEmployees.filter(e => e.id !== employeeId));
    };    
    ...
    <div className='wrap'>
        ...
        <div className='content'>
            ...
            <AddSubjectEmployeeModal 
                isOpen={isModalOpen}
                onClose={() => setModalOpen(false)}
                existingEmployees={subjectEmployees.map(employee => employee.id)}
                subjectId={id}
                onAddSuccess={handleAddSuccess}
            />


            <div className='subject-manage-info'>
                <img src={'http://localhost:8000/user_pfp/' + subjectData.professor_id + '.jpg'} alt="Selected professor" className="subject-manage-professor-image"/>
                <div className='subject-manage-info-details'>
                    <p className='subject-manage-subject-name'>{subjectDataDisplay.name}</p>
                    <p className='subject-manage-subject-code-year'>{subjectDataDisplay.code} - {subjectDataDisplay.year}</p>
                </div>
            </div>

            <div className='subject-manage-wrap'>
                <div className='subject-manage-left-column'>
                    <h2>Upravljanje zaposlenima</h2>

                    <div className='subject-manage-employees'>
                        {subjectEmployees.map((employee) => (
                            <SingleSubjectEmployee
                                key={employee.id}
                                employee={employee}
                    ...
                    <button className='subject-manage-add-employee-button' onClick={() => setModalOpen(true)}>DODAJ ZAPOSLENOG</button>
                </div>

                <div className='subject-manage-right-column'>
                    <h2>Upravljanje opštim parametrima</h2>

                    <form className='subject-manage-form' onSubmit={handleSubmit}>
                        ...
                        <label htmlFor="code">Šifra predmeta</label>
                        <input id="code" type="text" name="code" placeholder="Unesite šifru predmeta" value={subjectData.code} onChange={handleInputChange} />

                        <label>Odabir predmetnog profesora</label>
                        <div className="subject-manage-search-bar" style={{ position: 'relative' }}>
                            <input
                                type="text"
                                className="subject-manage-search-bar-input"
                            ...
                            <div className="select-course-and-year">
                                ...
                                <div className="selects-row">
                                    <Select
                                        ...
                                        isClearable={true}
                                        className="subject-manage-course-select"
                                    ...
                                    <Select
                                        ...
                                        styles={customSelectStyles}
                                        placeholder="Godina"
                                        className="subject-manage-year-select"
                        ...
                        <button type="submit" className="subjet-manage-submit-button">SAČUVAJ PROMENE</button>
                    </form>
                </div>
            </div>
        </div>
